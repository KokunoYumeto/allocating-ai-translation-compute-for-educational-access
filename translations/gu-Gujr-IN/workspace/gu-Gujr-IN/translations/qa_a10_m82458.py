...
from qa_a10_m82458_math import run as run_math_checks
ROOT=Path(__file__).resolve().parents[2]
SRC=ROOT/'downloads/gu-Gujr-IN/a10-release/source/authority/source/modules/m82458/index.cnxml'
GU=Path(__file__).with_name('a10-m82458.gu.cnxml')
...
def main():
 ...
 assert len(ex)==179 and len(sol)==115 and len(media)==49
 assert all(re.search('[\u0a80-\u0aff]',e.get('alt',''))or not re.search('[A-Za-z]',e.get('alt',''))for e in media)
 numerical=run_math_checks(s,g)
 errata=json.loads(GU.with_name('a10-m82458-errata.gu.json').read_text(encoding='utf8'))
 corrected_shift=errata['entries']['eip-id1169749465627']
 assert corrected_shift['source_image'].endswith('CNX_ElemAlg_Figure_01_07_013a_img_new.jpg')
 assert corrected_shift['evidence_media'].endswith('CNX_ElemAlg_Figure_01_07_013a_img_new.jpg')
 assert 'eip-id1169751952546' not in errata['entries']
 ...
 report={'result':'pass','module':'m82458','elements':len(b),'source_ids':len(ids),'mathml':len(g.findall('.//m:math',NS)),'exercises':len(ex),'source_solutions':len(sol),'source_omitted_solutions':len(ex)-len(sol),'media_alternatives_checked':len(media),'translated_media_alt':sum(bool(re.search('[\u0a80-\u0aff]',e.get('alt','')))for e in media),'translated_aria_labels':sum(bool(e.get('aria-label'))for e in b),'translated_summaries':sum(bool(e.get('summary'))for e in b),'source_title_attributes':sum('title'in e.attrib for e in a),'errata_entries':len(errata['entries']),'source_sha256':hashlib.sha256(SRC.read_bytes()).hexdigest(),'translation_sha256':hashlib.sha256(GU.read_bytes()).hexdigest(),'numerical':numerical}
 (ROOT/'gu-Gujr-IN/reviews/a10-m82458-qa.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
