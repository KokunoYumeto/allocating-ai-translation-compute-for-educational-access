...
from qa_a10_m82460_math import run as run_math_checks
...
def main():
 ...
 assert len(ex)==156 and len(sol)==105 and len(media)==17
 assert all(re.search('[\u0a80-\u0aff]',e.get('alt',''))or not re.search('[A-Za-z]',e.get('alt',''))for e in media)
 numerical=run_math_checks(s,g)
 errata=json.loads(GU.with_name('a10-m82460-errata.gu.json').read_text(encoding='utf8'))
 ...
 report={'result':'pass','module':'m82460','elements':len(b),'source_ids':len(ids),'mathml':len(g.findall('.//m:math',NS)),'exercises':len(ex),'source_solutions':len(sol),'source_omitted_solutions':len(ex)-len(sol),'media_alternatives_checked':len(media),'translated_media_alt':sum(bool(re.search('[\u0a80-\u0aff]',e.get('alt','')))for e in media),'translated_aria_labels':sum(bool(e.get('aria-label'))for e in b),'translated_summaries':sum(bool(e.get('summary'))for e in b),'source_title_attributes':sum('title'in e.attrib for e in a),'errata_entries':len(errata['entries']),'source_sha256':hashlib.sha256(SRC.read_bytes()).hexdigest(),'translation_sha256':hashlib.sha256(GU.read_bytes()).hexdigest(),'numerical':numerical}
 (ROOT/'gu-Gujr-IN/reviews/a10-m82460-qa.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
