"""Complete source-bound Gujarati translation of A10 m82457.
...
SRC=ROOT/'downloads/gu-Gujr-IN/a10-release/source/authority/source/modules/m82457/index.cnxml'
MAP=Path(__file__).with_name('a10-m82457.slots.json')
TSV=Path(__file__).with_name('a10-m82457.gu.tsv')
OUT=Path(__file__).with_name('a10-m82457.gu.cnxml')
SHA='a8db22ca9c8c10c7f95fa3aa4dcb18596711741478515c220a75e291273983e9'
...
def polish(root,source):
    ...
    for a,b in zip(source.iter(),root.iter()):
        ...
        if (a.tail or '').strip()=='To evaluate':b.tail='આપેલી પદાવલી '
        if (a.tail or '').strip() in {'substitute','we substitute'}:b.tail=' પદાવલીમાં મૂકવાની કિંમત છે '
        if (a.tail or '').strip()=='for':b.tail='; આ કિંમત આ ચલની જગ્યાએ મૂકો: '
        if (a.tail or '').strip()=='in the expression.':b.tail='.'
    for a,b in zip(source.iter(),root.iter()):
        if a.tag==c+'item' and len(a)>1 and any((x.tail or '').strip()=='To evaluate' for x in a):
            assert a[0].tag==c+'span' and a[1].tag==m+'math'
            b[0].tail=' પદાવલી ';b[1].tail='ની કિંમત શોધવા, જ્યારે '
    rewrite('fs-id1170654967245','આ ',['માં લઘુત્તમ સામાન્ય છેદ 36 માટે 2ના બે અવયવો અને આ સંખ્યાના બે અવયવો છે: ',None])
    rewrite('fs-id1170655189510','હવે આપણે અપૂર્ણાંકો પરની ચારેય ક્રિયાઓ જાણીએ છીએ. ',['માં ', 'નો સારાંશ છે.'])
    rewrite('fs-id1170654969636','આપણે જોયું છે કે જટિલ અપૂર્ણાંકના અંશ કે છેદમાં અપૂર્ણાંક હોય છે. અપૂર્ણાંક રેખા ',[' દર્શાવે છે. આપણે આ જટિલ અપૂર્ણાંકને સાદું રૂપ આપ્યું: ', ' તેને ભાગાકાર તરીકે લખીને, ', 'ને ', ' વડે ભાગ્યું.'])
    rewrite('fs-id1170655189822','પદાવલી ',['ની કિંમત શોધવા આપેલી ચલકિંમતો વાપરીએ: ', ' આ કિંમતો પદાવલીમાં મૂકીએ.',None,None])
    errata=Path(__file__).with_name('a10-m82457-errata.gu.json')
    if errata.exists():
