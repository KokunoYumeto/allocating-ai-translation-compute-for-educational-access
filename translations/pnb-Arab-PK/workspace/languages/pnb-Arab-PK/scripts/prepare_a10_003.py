"""Prepare only the two manifest-declared A10-003 original JPEGs."""
...
def load_inputs():
    ...
    require((manifest['unit'], manifest['assignment'], manifest['collection'], manifest['module'])
            == ('A10-003', 'A10', 'col31130', 'm82452'), 'Unexpected A10-003 scope')
    ...
    excerpt_path = BASE / 'source-excerpts/a10-unit-003.cnxml'
    ...
    require(manifest['top_level_content_child_start_zero_based'] == 3
            and manifest['top_level_content_children_included'] == 2, 'Changed top-level scope')
    selected = list(content)[3:5]
    require([node.get('id') for node in selected] == manifest['selection_ids']
            == ['newelem_para01', 'fs-id1170655199097'], 'Skipped immediate source paragraph or section')
    section = selected[1]
    require(section.get('id') == manifest['section_start'] and len(section) == 17
            and manifest['section_direct_children_included'] == 17, 'Changed complete-section extent')
    require(selected[0].get('id') == manifest['scope_from']
            and section[-1].get('id') == manifest['scope_through_inclusive']
            == 'fs-id1170655200179', 'Changed selected boundaries')
    require(list(content)[5].get('id') == manifest['next_source_id'] == 'fs-id1170655247410',
            'Changed next source section')
    require(len(excerpt) == 2 and [tree(node) for node in selected] == [tree(node) for node in excerpt],
            'Frozen complete paragraph and section differ from canonical source')
    child_keys = [node.get('id') or section.get('id') + '/title' for node in section]
    require(child_keys == manifest['selected_section_child_ids'], 'Changed selected child sequence')
    ids = [node.get('id') for node in excerpt.iter() if node is not excerpt and node.get('id')]
    require(ids == manifest['source_ids_in_document_order'] and len(ids) == 40, 'Changed source ID sequence')
    parents = {child: node for node in excerpt.iter() for child in node}
    images = [node for node in excerpt.iter() if tag(node) == 'image']
    require(len(images) == len(manifest['images']) == 2, 'Only two selected images are allowed')
    ...
    require(sum(len(item[3]) for item in prepared) == manifest['total_declared_image_bytes'] == 71792,
...
def notice_record(manifest, prepared, notices):
    return {
        ...
        'unit': 'A10-003', 'work': 'OpenStax Elementary Algebra 2e',
        ...
        'scope': 'Two unchanged declared source images only; no new supply or license audit.',
        ...
        'images': [
            dict(figure_id=spec['figure_id'], media_id=spec['media_id'],
                 ...
                 treatment='Original JPEG bytes unchanged and unmirrored; faithful Punjabi alt with separately labeled original symbol-reading key.',
...
def prepare():
    ...
    print('Prepared A10-003: 2 original JPEGs, 71,792 bytes; existing rights status retained, no new clearance.')
