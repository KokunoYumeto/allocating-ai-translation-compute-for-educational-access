...
from recovery_io import BASE,ROOT,MANIFEST,TRANSLATION,WITNESS,NOTICES,ASSETS,load,require,file_hash
...
class Reader:
    ...
    def originals(self):
        ...
        out+='<h3>شروع دے عارضی اصطلاحی چُون</h3><p class="term-intro">ایہہ فنی ناں عارضی چُون نیں۔ اصل اصطلاحی نوٹ <a href="../translations/b40-opening.json">محفوظ ترجمے دے ریکارڈ</a> وچ موجود نیں۔</p><dl class="term-grid">'
        for row in self.t["provisional_terms"]:
            out+='<div class="term-row"><dt lang="en" dir="ltr">'+esc(row["english"])+"</dt><dd>"+esc(row["punjabi"])+"</dd></div>"
