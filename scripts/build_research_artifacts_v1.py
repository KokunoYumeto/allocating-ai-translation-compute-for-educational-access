...
The source of truth remains the supplied manuscript.  This builder deliberately
implements the bounded Markdown used by that manuscript: headings, paragraphs,
emphasis, inline code, ordered/unordered lists, pipe tables, display equations,
images, block quotes, and figure/table captions.  The complete APA register can
be injected from a separately hashed Markdown file so that the editable edition
contains the same references as the evidence package.
...
The script contains no publication logic and changes no source files.  It writes
only the requested DOCX and optional JSON build report.
"""
...
def sha256_file(path: Path) -> str:
    """Return a stable SHA-256 digest for a file without loading it all at once."""
...
def normalize_docx_zip(path: Path) -> None:
    """Normalize OOXML ZIP member order/timestamps for reproducible builds."""
    temp_name: str | None = None
    try:
        with tempfile.NamedTemporaryFile(
            prefix="docx-normalize-", suffix=".docx", dir=str(path.parent), delete=False
        ) as temp:
            temp_name = temp.name
        with zipfile.ZipFile(path, "r") as source, zipfile.ZipFile(
            ...
            for info in sorted(source.infolist(), key=lambda item: item.filename):
                ...
                normalized = zipfile.ZipInfo(info.filename, date_time=(2026, 9, 1, 0, 0, 0))
...
def configure_styles(doc: Document) -> None:
    ...
    if "Research Quote" not in styles:
        quote = styles.add_style("Research Quote", WD_STYLE_TYPE.PARAGRAPH)
    else:
        quote = styles["Research Quote"]
    set_style_font(quote, "Calibri", 11, PRESET["colors"]["ink"])
    ...
    quote.paragraph_format.line_spacing = 1.20
...
def parse_markdown(text: str) -> tuple[dict[str, str], list[Token]]:
    ...
    metadata = {
        ...
        "subtitle": "Evidence-bound methods for language, stage, subject, and accessibility allocation",
        "date": "",
    }
    ...
    for line in lines[1:12]:
        ...
        freeze_match = re.search(r"\*\*Evidence freeze:\*\*\s*(.+)", line)
        if freeze_match and not metadata["date"]:
            metadata["date"] = "Evidence freeze - " + freeze_match.group(1).strip().rstrip("  ")
...
def parse_reference_tokens(path: Path) -> list[Token]:
    """Parse the standalone APA register using the same bounded Markdown reader.

    The register has its own title and section headings.  The outer manuscript
    already supplies the navigational ``References`` heading, so the register's
    document title is omitted while its topical section headings, citations, and
    claim-boundary notes are retained.
    """
    if not path.exists():
        return []
    _, tokens = parse_markdown(path.read_text(encoding="utf-8-sig"))
    filtered: list[Token] = []
    for token in tokens:
        if token.kind == "heading" and token.level == 1 and not filtered:
            # First heading is the register's own title.
            continue
        if token.kind == "heading" and str(token.value).strip().lower() == "references":
            continue
        if token.kind == "heading":
            # Keep register sections nested under the manuscript's References
            # heading so the renderer retains the Reference Entry style.
            token = Token("heading", token.value, min(3, token.level + 1))
        filtered.append(token)
    return filtered


def inject_reference_register(tokens: Sequence[Token], reference_path: Path | None) -> list[Token]:
    """Replace the manuscript's pointer paragraph with the complete register."""
    if reference_path is None or not reference_path.exists():
        return list(tokens)
    # Newer publication manuscripts already carry their cited APA bibliography
    # inline.  In that case the standalone evidence register remains a source
    # artifact, but injecting it here would duplicate the bibliography and add
    # uncited contextual entries to the rendered paper.
    ...
    if reference_heading is not None:
        inline_reference_paragraphs = [
            str(token.value)
            for token in tokens[reference_heading + 1 :]
            if token.kind == "paragraph"
        ]
        if any(
            re.search(r"\(\d{4}[a-z]?\)\.", paragraph)
            and ("http://" in paragraph or "https://" in paragraph)
            for paragraph in inline_reference_paragraphs
        ):
            return list(tokens)
    register = parse_reference_tokens(reference_path)
    if not register:
        return list(tokens)
    expanded: list[Token] = []
    in_references = False
    ...
    for token in tokens:
        if token.kind == "heading" and str(token.value).strip().lower() == "references":
            ...
            expanded.extend(register)
            injected = True
            in_references = True
            continue
        if in_references:
            if token.kind == "heading" and token.level <= 1:
                in_references = False
            elif token.kind == "paragraph" and "complete apa-style reference register" in str(token.value).lower():
                # The pointer is true but redundant once the register is embedded.
                continue
        expanded.append(token)
    return expanded if injected else list(tokens)
...
MATH_CACHE: dict[tuple[str, bool], bytes] = {}


def tex_to_omml(source: str, *, display: bool) -> object | None:
    """Convert bounded TeX math to native Word OMML through local Pandoc.

    The report contains only a small fixed equation set.  Native OMML avoids
    publishing visible TeX commands while keeping equations editable and
    accessible to Word/LibreOffice.  Cached XML makes repeated identifiers
    inexpensive and deterministic; a conservative text fallback remains below.
    """
    key = (source, display)
    if key in MATH_CACHE:
        return etree.fromstring(MATH_CACHE[key])
    pandoc = shutil.which("pandoc")
    if not pandoc:
        local_appdata = os.environ.get("LOCALAPPDATA")
        candidate = Path(local_appdata) / "Pandoc" / "pandoc.exe" if local_appdata else None
        if candidate and candidate.exists():
            pandoc = str(candidate)
    if not pandoc:
        return None
    markdown = f"$${source}$$" if display else f"${source}$"
    try:
        with tempfile.TemporaryDirectory(prefix="publication_math_") as temp_dir:
            output = Path(temp_dir) / "math.docx"
            subprocess.run(
                [pandoc, "--from=markdown+tex_math_dollars", "--to=docx", "--output", str(output)],
                input=markdown,
                text=True,
                encoding="utf-8",
                capture_output=True,
            ...
            tag = f"{{{MATH_NS}}}oMathPara" if display else f"{{{MATH_NS}}}oMath"
            node = root.find(f".//{tag}")
            if node is None:
                return None
            payload = etree.tostring(node)
            MATH_CACHE[key] = payload
            return etree.fromstring(payload)
    except (OSError, subprocess.SubprocessError, zipfile.BadZipFile, etree.XMLSyntaxError):
        return None


def add_hyperlink(paragraph, label: str, target: str, *, size: float | None = None) -> None:
    """Append a real external hyperlink while retaining the visible label.

    python-docx has no public hyperlink API.  The small OOXML fragment below is
    deliberately explicit so screen readers and exported PDFs retain a usable
    destination rather than a decorative, non-clickable URL.
    """
    target = target.strip()
    if not re.match(r"^https?://", target, flags=re.I):
        run = paragraph.add_run(label)
        set_run_font(run, size=size, color="2E74B5")
        run.underline = True
        return
    rid = paragraph.part.relate_to(target, RELATIONSHIP_TYPE.HYPERLINK, is_external=True)
...
def add_inline(paragraph, text: str, *, default_size: float | None = None) -> None:
    ...
    for match in INLINE_RE.finditer(text):
        ...
        if token.startswith(r"\("):
            math_node = tex_to_omml(token[2:-2], display=False)
            if math_node is not None:
                paragraph._p.append(copy.deepcopy(math_node))
            else:
                # Fail legibly if Pandoc is unavailable rather than dropping
                # mathematical content or printing delimiter characters.
            ...
            add_hyperlink(paragraph, label, target, size=default_size)
        elif token.startswith(("http://", "https://")):
            # Keep sentence punctuation outside the relationship target.
            trailing = ""
            while token and token[-1] in ".,;:!?)]":
                trailing = token[-1] + trailing
                token = token[:-1]
            add_hyperlink(paragraph, token, token, size=default_size)
            if trailing:
                tail = paragraph.add_run(trailing)
                if default_size:
                    set_run_font(tail, size=default_size)
        else:
            run = paragraph.add_run(token[1:-1])
...
def add_equation(doc: Document, source: str) -> None:
    ...
    math_node = tex_to_omml(source, display=True)
    if math_node is not None:
        paragraph._p.append(copy.deepcopy(math_node))
        return
...
def table_is_wide(rows: Sequence[Sequence[str]], context: str) -> bool:
    ...
    if context in {"appendix_b", "appendix_c"}:
        return True
    # Keep the compact six-column decision tables in portrait.  Moving a short
    # table and its surrounding subsection into a new landscape section can
    # strand the final sentence of the preceding section on a nearly blank
    # page.  The current manuscript's genuinely dense tables are either
    # seven-column+ or explicitly marked appendix tables and remain landscape.
    if columns >= 7:
        return True
...
def orientation_plan(tokens: Sequence[Token]) -> list[str]:
    ...
    for table_index, token in enumerate(tokens):
        ...
        for i in range(heading_index, end):
            plan[i] = "landscape"
    # The overlap-control paragraph closes 6.2 conceptually but directly frames
    # the wide compute-order table in 6.3.  Move it with that landscape plate so
    # the orientation break cannot strand it on an otherwise empty page.
    for index, token in enumerate(tokens):
        if token.kind == "paragraph" and str(token.value).startswith(
            "Taiwan's official 2020 table reports"
        ):
            plan[index] = "landscape"
    # Reproducibility and limitations form one continuous analytic plate.
    # Keeping them landscape prevents a short tail paragraph from being
    # stranded when the report returns to portrait for references.
    for heading_index, token in enumerate(tokens):
        if token.kind != "heading" or token.level != 1:
            continue
        if not re.match(r"^(9\. Reproducibility|10\. Limitations)\b", str(token.value)):
            continue
        end = next(
            (
                i
        ...
        for i in range(heading_index, end):
            plan[i] = "landscape"
    # Section 6.6 contains the wide package table.  Keep the short 6.7 delivery
    # subsection in the same landscape run so a section break does not strand
    # the preceding table's closing prose on a nearly empty page.
    for heading_index, token in enumerate(tokens):
        if token.kind != "heading" or not str(token.value).startswith(
...
def add_body_paragraph(doc: Document, text: str, context: str) -> None:
    is_quote = bool(re.match(r"^>\s*", text.strip()))
    source_text = re.sub(r"^>\s*", "", text.strip()) if is_quote else text
    plain = re.sub(r"[*`]", "", source_text).strip()
    is_caption = bool(re.match(r"^(Table\s+\d|Table\s+\d+[A-Z]|Figure\s+\d)", plain, re.I))
    if is_caption:
        paragraph = doc.add_paragraph(style="Caption")
    elif is_quote:
        paragraph = doc.add_paragraph(style="Research Quote")
        set_paragraph_border(paragraph, "left", PRESET["colors"]["gold"], size=18)
        set_paragraph_shading(paragraph, "FFFDF5")
    ...
    add_inline(paragraph, source_text)
    paragraph.paragraph_format.widow_control = True
...
def build_document(
    ...
    metadata, parsed_tokens = parse_markdown(source.read_text(encoding="utf-8-sig"))
    tokens = inject_reference_register(parsed_tokens, references)
    if not metadata.get("date"):
        metadata["date"] = "Reproducibility snapshot - 1 September 2026"
    layout_plan = orientation_plan(tokens)
    doc = Document()
    configure_styles(doc)
    numbering = NumberingFactory(doc)

    # A fixed package timestamp keeps repeated builds byte-stable apart from
    # intentional source changes.  The source snapshot date is printed on the
    # cover; this timestamp is only OOXML metadata.
    fixed_time = datetime(2026, 9, 1, 0, 0, tzinfo=timezone.utc)
    props = doc.core_properties
    props.title = metadata["title"]
    props.subject = metadata.get("subtitle", "")
    props.author = MODEL_ID
    props.last_modified_by = MODEL_ID
    props.category = "Research report"
    props.version = "from_scratch_20260901"
    props.content_status = "Draft for deterministic QA"
    ...
    for index, token in enumerate(tokens):
        ...
        if token.kind == "heading":
            ...
            if orientation == "landscape":
                ...
                if heading_text == "10. Limitations":
                    # This closing heading follows the release table on the
                    # same analytic plate; compact spacing avoids a two-line
                    # orphan immediately before the appendix section break.
            ...
            if orientation == "landscape":
                ...
                if str(token.value).startswith("Academic-language comprehension"):
                    # The compact closing limitations paragraph belongs to the
                    # full-width reproducibility plate.  Letting it use the
                    # plate width prevents a two-line spill before Appendix A.
                    doc.paragraphs[-1].paragraph_format.right_indent = Inches(0)
    ...
    return {
        ...
        "references": str(references.resolve()) if references else None,
        "source_sha256": sha256_file(source),
        "references_sha256": sha256_file(references) if references and references.exists() else None,
        "model": MODEL_ID,
...
def main() -> int:
    ...
    parser.add_argument(
        "--references",
        type=Path,
        help="Optional complete APA Markdown register (defaults to ../sources/REFERENCES_APA.md)",
    )
    ...
    references = args.references.resolve() if args.references else (source.parent / "sources" / "REFERENCES_APA.md")
    if not references.exists():
        references = None
    output = args.output.resolve()
    if source == output:
        raise SystemExit("Input and output must differ")
    report = build_document(source, figure, output, references)
