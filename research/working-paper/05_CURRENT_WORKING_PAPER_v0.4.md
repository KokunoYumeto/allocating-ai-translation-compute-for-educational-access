...
This working paper asks how many people could plausibly gain such access; it
does not rank the worth of languages or communities. The current comparison
keeps target-comparable research wherever it exists and applies the same
disclosed baseline where an equivalent academic-language measurement is
missing. Under that common missing-data assumption, the largest displayed
non-English opportunities begin with Mandarin Chinese, Hindi, Spanish,
Standard Arabic, French, Bengali, Portuguese, Indonesian, Urdu and Russian.
...
research obligation, never evidence of low need.

## Current checkpoint: population plausibly helped

The short table below is a **common-assumption reference**, not a funding
league table. It answers one narrower question: if the same share of each
language's users cannot comfortably and independently use the academic-English
register of a source, how many people could plausibly benefit from a
source-faithful edition?

The point seed is 89.57%, the population-weighted share who did not report
...
This is a transparent imputation, not a global estimate of academic-English
proficiency: the source measures reported language use, India is not a global
sample, and academic English is a more demanding task than ordinary English.
Target-specific research remains in the paper. It replaces the common
imputation only when it measures the same population, educational task,
register threshold, stage and material endpoint.

| Reference order | Language / first edition profile | Rounded total usage in source | Plausibly helped at common point | Sensitivity range |
|---:|---|---:|---:|---:|
| 1 | Mandarin Chinese / Simplified Standard Written Chinese first profile | 1.2 billion | 1.075 billion | 840.0 million-1.188 billion |
| 2 | Hindi and separately retained Hindi-belt varieties | 611.2 million | 547.5 million | 427.8-605.1 million |
| 3 | Spanish with territorial and adult-register profiles | 561.3 million | 502.8 million | 392.9-555.7 million |
| 4 | Modern Standard Arabic plus separately retained vernaculars | 334.9 million | 300.0 million | 234.4-331.6 million |
| 5 | French with territorial and adult-register profiles | 333.5 million | 298.7 million | 233.4-330.2 million |
| 6 | Bangla/Bengali with Bangladesh and India profiles | 274.4 million | 245.8 million | 192.1-271.7 million |
| 7 | Portuguese with Brazil, Portugal and African profiles | 269.4 million | 241.3 million | 188.6-266.7 million |
| 8 | Indonesian | 254.8 million | 228.2 million | 178.4-252.3 million |
| 9 | Urdu with Pakistan and India profiles | 246.0 million | 220.4 million | 172.2-243.5 million |
| 10 | Russian with territorial and diaspora profiles | 210.3 million | 188.4 million | 147.2-208.2 million |

These rows are high-reach starting points because their source populations are
large under one deliberately equal assumption. They do not show that a
complete course, a short companion and a research monograph have the same
effect. Nor do they imply that a smaller language has less severe need, fewer
rights, or lower per-person benefit. The full universal register is the
inclusion boundary; the table is only one readable headcount view.

Ukrainian remains explicit rather than disappearing because a matching
academic-English measurement has not yet been found. Its current rounded
total-usage source value is 30.8 million, giving a common-point reference of
27.6 million and a sensitivity range of 21.6-30.5 million. Russian knowledge
does not automatically count as acceptable coverage. Any deduction would
require the same material to be technically usable, linguistically sufficient,
voluntarily and politically acceptable, accessible, affordable, safe and
actually available. The same rule applies worldwide to colonial languages,
state languages, regional lingua francas, diaspora languages and other
multilingual routes.

The complete current 21-row checkpoint, exact source bindings, formulas and
retained research context are published in
[HYBRID_LANGUAGE_REACH_BASELINE_v1.md](HYBRID_LANGUAGE_REACH_BASELINE_v1.md)
and its machine-readable CSV and JSON companions.
...
The globally decisive portfolio is not yet complete. A bounded conditional
ten-program tranche and a 100-profile communication portfolio now exist, but
they are a diversified research-and-production hedge rather than a discovered
world ranking. Required remaining work includes language-route estimates around
stage-specific populations; item-level comparisons of independently usable
subject provision; measured effects; standardized physical-compute scenarios;
global omission and sensitivity tests; and comparisons against possible
outrankers. Signed-language and accessibility interventions, smaller
communities, and vocational and research opportunities remain substantive
lanes rather than residual categories.
...
The requested Top 10 and Top 100 have now been constructed only in this
conditional sense. The ten-program tranche is one feasible hedge across large
audiences, regional depth, displacement, multiple scripts and practical
subject needs. The 100-profile set is a wider communication and investigation
portfolio. Neither is an ordinal result, a unique solution or an eligibility
boundary. The frozen 100, the 169 current challengers and all entities outside
those displays remain possible outrankers. Each selection must publish its
assumptions, the unmeasured candidates that could displace it and the evidence
most likely to change the composition. Accessibility formats and interlanguages
remain parallel, nonadditive design layers; all 209 origin and 178 host
displacement contexts remain visible rather than being reduced to ten examples.
...
The replacement synthesis now has a route-level evidence funnel rather than an implied league table. It preserves 1,022 proposed language-stage-package routes. Fifteen have a numeric need interval or point in a source-defined context; thirty-two have a declared base text-traffic scenario; and nine routes participate in thirty-six same-endpoint threshold comparisons. Those are useful conditional comparisons, but they are not global winners. A national cohort, census category or status population is not automatically the exact audience that can use one edition. A token-traffic scenario is not standardized physical compute, energy, money or achieved quality.
...
The next allocation step is therefore specific, not vague: complete comparable cells for exact language/script/community × stage × subject × package × delivery interventions; estimate standalone and overlapping benefits; normalize the complete production and delivery workflow to physical compute; and then publish only interval-supported dominance relations. Any Top 10 or Top 100 must show the assumptions for each member, the candidates that can still outrank it and the evidence most likely to change the selection.
...
The wider 100-profile portfolio is likewise a communication set, not ranks
1–100. Its member numbers are record identifiers. All 10,525 analytical entities
outside it remain possible outrankers, and every slot is replaceable when new
evidence changes the portfolio. The current partial order still contains zero
robust global dominance edges because no pair jointly binds compatible audience,
same-endpoint effect, standardized physical compute and nonoverlapping
