...
AI-assisted translation can expand educational access when it moves complete
material into a language, register and format that people can actually use.
This working paper asks how a fixed production-compute budget can create the
largest additional functional educational understanding. It does not rank the
worth of languages or communities. The analysis first estimates who may be
reached without deleting under-researched communities, then compares concrete
language × stage × subject × package × format interventions on compatible
educational endpoints. Under the disclosed common missing-data assumption, the
largest displayed non-English reach opportunities begin with Simplified
Standard Written Chinese, Hindi, Spanish, Arabic, French, Urdu, Bangla,
Portuguese, Indonesian and Russian.
Simplified Standard Written Chinese is the first proposed writing profile for
the Mandarin row, not a claim that all Sinitic communities use one edition.
English is also an access target: ordinary English use does not establish
comfortable independent use of academic English, so adult register-to-register
translations remain a separate, currently unmeasured global opportunity.

The paper compares language, subject, educational stage and delivery format
from early learning through research. It retains every language when evidence
is missing, separates population plausibly helped from learning effect and
production cost, and gives no automatic coverage credit to a colonial,
national, prestige or geopolitically coercive language. Initial source
comparisons identify substantial foundational and secondary opportunities,
research-stage language burdens, accessibility needs, and distinct routes for
signed, displaced, local-language and independent learners. The new
register-translation method preserves every proposition, definition,
quantifier, proof dependency, number, citation and uncertainty marker while
changing how the content is expressed for a specified adult audience. The
 worldwide synthesis is ongoing and non-exhaustive; missing evidence is a
 research obligation, never evidence of low need. Fixed-budget calculations
 compare language editions, shared preparation and different package sizes.
 In twelve declared scenarios, mixing focused resources and complete books
 improves the modelled opportunity score by zero to 22.6% over the best
 policy using one package size. This is a scenario result, not a measured
 learning gain. A separate nine-route secondary-school proficiency analysis
 provides educational context; it does not replace worldwide language
 audiences or measure the benefit of a particular translation.

## Current checkpoint: worldwide reach and a fixed-package compute proxy

The public comparison now contains 775 non-English language-and-script edition
profiles rather than a short convenience sample. The first table below shows
the first ten of a real 100-row potential-reach table. A separate 100-row table
then divides the same reach numerator by the estimated token-event workload for
one identical 100,000-source-token text package and one nominal quality target.
Stable profile identifiers are never positions.

The point seed remains 89.57%, the population-weighted share who did not report
English across 17 India Census 2011 C-17 mother-tongue rows:
1,044,314,726 of 1,165,872,176 people. The sensitivity range is 70% to 99%.
This is a transparent preservation assumption, not an observed worldwide rate
of academic-English access. It keeps an under-researched community visible.
Comparable target research replaces it only when audience, educational task,
register threshold, stage and material endpoint match.
...
The Indonesian point retains the direct 2022 BPS estimate of people aged five
and older able to understand and produce comprehensible Indonesian; it does not
equate oral ability with academic reading or package effect. The other rows use
CLDR 48 interface-fluent L1/L2 estimates resolved to likely scripts. CLDR is a
locale-planning source, not a complete census of independently usable academic
editions. Multilingual, macrolanguage/member and multiscript rows overlap and
cannot be summed into a world total.

Under the fixed-package workload proxy, the first ten are Simplified Standard
Written Chinese, Hindi, Spanish, Arabic, French, Urdu, Portuguese, Indonesian,
Bangla and Russian. Indonesian moves from reach position 9 to proxy position 8;
Chinese remains first. These positions do not yet measure learning effect,
delivery, physical compute, price, shared terminology, accessibility work or
overlapping beneficiaries. Fresh-equivalent token events are workload scenarios,
not hardware operations, energy, time, money or quality.

Ukrainian is present at reach position 62 and fixed-package proxy position 62
under the pinned CLDR layer: 24.1 million interface-fluent users and 21.6 million
at the common point. That source omits part of the displaced diaspora, so this
is not a final addressable-audience estimate. Russian knowledge receives no
automatic coverage credit. Any overlap deduction requires the same material to
be technically sufficient, linguistically usable, socially and politically
acceptable, accessible, safe and actually available. The same rule applies
worldwide to colonial, state, prestige and regional lingua-franca routes.

The complete two 100-row tables, all 775 rows, exact source and workload
bindings, formulas and limitations appear in the appendix **Worldwide reach and
common-package allocation checkpoint** and in the accompanying CSV and JSON
files. They restore the quantitative allocation chain; they do not replace the
target-specific educational-effect and physical-compute research still needed.

## Current checkpoint: a nine-route PISA-context workload sensitivity

The catalogue and the worldwide reach screen are not, by themselves, the allocation answer. The allocation question remains active and is answered only for explicitly declared scenario units below.
The decision unit is a concrete intervention: one language or register, one
educational stage and endpoint, one package and format, one delivery route and
one time horizon. The first checked subset in which a common context measure is
available contains nine secondary quantitative-catch-up routes. For route *i*,
the current conditional coefficient is `N_i/C_i`, where `N_i` is a
source-defined prevalence or context count and `C_i` is the audited token-event
workload. It is not yet a measure of people helped.
Actual additional access per workload is `(N_i/C_i)q_i`, where `q_i` is the
still-unmeasured average net probability that a person in that context gains the
specified endpoint because of the intervention.
...
This is a conditional comparison, not a claim that Indonesian currently has
the largest real effect. For example, Brazilian Portuguese exceeds Indonesian
if its target-specific `q` is more than about 1.85 times as large; Filipino does
so above about 2.42 times. Those ratios are reversal-threshold sensitivities, not
value-of-information estimates or a measurement-priority ranking: no prior,
measurement likelihood, decision utility or measurement cost has been specified.
Thai, Vietnamese, Standard Malay, Japanese, Central Khmer and Ukrainian remain
possible outrankers if their target-specific net yields are sufficiently higher.
The thresholds show exactly how much the unknown effect would have to differ; they
do not rank people or entitlement to educational material.

The effect term is being bound through six observable stages: exact-audience
fit, reach, acquisition, practical usability, active use and endpoint gain,
minus any endpoint loss. This factorization is a measurement plan, not an
independence assumption; a directly comparable intent-to-treat estimate would
replace the product. Every component is currently recorded as unmeasured rather
than zero. Fifteen routes with numeric context remain in the frontier; six use
different endpoints and are retained without being forced into this
cross-ranking. All 1,022 candidate routes remain possible outrankers.

Token-event workload is still not physical compute. Provider accelerator work
cannot be recovered from cached-input, fresh-input, output-token, elapsed-time
or price counters. The final denominator will therefore use a declared reference
replay: accelerator-seconds per accepted package plus full-system joules, under
cold-isolated and stated operational scenarios. Until that replay exists, the
physical-compute field is explicitly unidentified.

## Current checkpoint: academic English is also a translation target

The English source population is 1.5 billion in the same rounded source, but
the share unable to use academic English for a specified task is not measured
globally. The honest expression is therefore
**1.5 billion multiplied by q for the relevant academic-register mismatch**.
English-first-language identity, conversational English, years of schooling,
or conformity to British or American prestige norms cannot set that mismatch
to zero.

The governing premise is a sufficiently plausible access thesis, not a
go/no-go hypothesis: inherited command of academic register is one
institutionally distributed advantage, and any credible nonzero educational
benefit makes a source-faithful adult register edition eligible. Research
determines how the translation should be designed, checked, measured and
revised. It may not turn correlations among schooling, family education,
income, test performance, language exposure or attainment into claims of
innate capability or reasons to withhold translation.

The method is **full-content diaphasic intralingual translation**: translation
between registers within English for a declared adult audience. It is not
abridgment, remediation, child-directed explanation, or replacement of the
original. The target may be longer. It may unpack dense grammar, define terms,
split overloaded sentences, make logical relations explicit, resolve
references, reorder with tracked dependencies, and add clearly labeled
examples or background. It may not omit, generalize, silently strengthen or
weaken, or substitute intuition for a proof.

There is no single universal Plain English. The current design distinguishes
at least international/ELF-friendly adult English, educated non-specialist
English, profession-adjacent novice English, localized adult Englishes, an
academic-convention companion, and separately commissioned accessibility
editions. Every version preserves the same protected content while making
different lexical, grammatical, pragmatic and navigation choices.

The evidence is supportive but appropriately bounded. Parent education and
socioeconomic position are associated with unequal educational trajectories;
qualitative work documents tacit institutional discourse and hidden rules;
academic-language knowledge predicts reading comprehension in several school
samples; experiments show that jargon and document design can change
processing or understanding; and one randomized university intervention found
that making background-linked challenges and navigation strategies explicit
improved first-generation outcomes. No identified study tests the complete
chain from parent education through full-content register translation to
long-run attainment. That gap defines what to measure; it does not veto the
intervention.

Ordinary text simplification is not an adequate production objective. One
document-level dataset reports deletion as 44% of sampled operations, and
factuality studies show omissions and unsupported additions that common
metrics miss. This project therefore requires bidirectional source-target
claim alignment and zero critical errors for definitions, propositions,
quantifiers, conditions, numbers, formulas, proof dependencies,
cross-references, attribution and epistemic status. Readability and similarity
scores are alarms, not proof of fidelity.

The full evidence synthesis, 42-source register, six target profiles, 13
protected content classes and 20-step production workflow are published in
[ACADEMIC_REGISTER_TRANSLATION_RESEARCH_v1.md](ACADEMIC_REGISTER_TRANSLATION_RESEARCH_v1.md).
The anti-hegemony model and deterministic counterfactual tests are published
in [HEGEMONY_BIAS_AUDIT_v1.md](HEGEMONY_BIAS_AUDIT_v1.md).
...
Language-of-instruction evidence helps explain how linguistic access can affect learning. The World Bank (2021, p. 9) estimates that 37% of students in low- and middle-income countries are not taught in the language they speak and understand best. That definition is not the same as being unable to understand any instruction. The report connects language policy with learning, persistence and unequal access (p. 9), and identifies scope for reaching many affected learners through additional written languages (p. 10). This motivates separate estimates of relevant audience, acquisition, use and educational gain. The cross-country prevalence figure is not itself a translation-effect estimate or a count of people who would benefit from one book.
...
Finally, the national Bangla route is not the whole country. The universal register contains 50 Glottolog language-level nodes linked to Bangladesh. Bengali (`ben`, `beng1280`) is the national written route considered here, but it cannot be counted as automatic coverage for Sylheti (`syl`), Chittagonian (`ctg`), Rohingya (`rhg`), signing communities or the other country-linked nodes. Glottolog currently links Indian Sign Language (`ins`, `indi1237`) to Bangladesh, India and Pakistan; that catalog identity does not establish that one standardized signed route serves every Bangladeshi signing community. Each remains a separate evidence obligation.
...
Existing Russian provision materially narrows some proposals. Yandex displays school mathematics and computing resources; Stepik's inspected introductory statistics page displayed 326,869 learners; Open Education displayed 1,459 courses and 419 programmes; MCCME provides full mathematical books; and Math-Net maintains journal archives. These are substantive alternatives, not proof of complete open licences, autonomous-study coherence, accessibility or zero additional need. They require a Russian package to identify the missing topic, prerequisite, answer support, format, rights or delivery advantage. They say nothing by themselves about the exact needs of Chechen, Tatar, Bashkir, Chuvash, Sakha, Tuvinian, Mari, Udmurt, Komi, Moksha, Ossetian, Nenets, Evenki, Chukchi, Nivkh or the other Russia-linked nodes.
...
The [Japan cross-stage checkpoint](JPN_CROSS_STAGE_SUPPLY_AND_PACKAGE_v1.md) tests a proposition that the earlier comparison left unresolved: strong aggregate educational performance does not make a country or language an appropriate zero-need control. Japan's 2025 population of 123,366,734, its large school and research systems, and its strong national PISA results establish scale and substantial existing provision. None is an exact count of people who would gain access from a particular Japanese, other-language, signed-language or accessibility package (MEXT, 2024a, 2024c; World Bank, 2026e).
...
National totals also hide locally important communities. In Alaska's state table, the source labels Central Yupik, Inupiaq and Central Siberian Yupik have estimated home-language populations of 15,680 ± 746, 5,722 ± 504 and 1,032 ± 116. Hawaiʻi's selected labels include Iloko at 59,050 ± 3,269, Hawaiian at 26,000 ± 1,916, Hawaiʻi Creole English at 11,600 ± 1,337, Chuukese at 7,861 ± 1,403 and Marshallese at 6,243 ± 1,163. These are Census labels, estimates and margins of error—not automatic Glottolog matches or course audiences. American Samoa, Guam, the Northern Mariana Islands, Puerto Rico, the U.S. Virgin Islands and the U.S. Minor Outlying Islands remain separate profiles. Retained Island Areas table shells prove that jurisdiction-specific language variables exist, but they do not contain the populated language counts; that missing evidence remains a priority rather than a zero (U.S. Census Bureau, 2022c, 2023a, 2023b).
...
Ghana shows a different measurement problem. Among 26,210,872 people aged six or older, 7,924,409 were reported not literate in any language and 18,286,463 literate. Of the latter, 9,662,938 were reported literate in a Ghanaian language, 17,610,870 in English and 183,443 in Hausa. These overlapping columns cannot be subtracted into exclusive readerships. The questionnaire separately names Akuapem Twi, Asante Twi, Fante, Nzema, Ga, Dangme, Ewe, Dagbani, Gonja, Dagaare, Kasem, Gruni and Hausa, but the inspected national table publishes most of them only inside the aggregate “Ghanaian language.” A separate ethnic table reports large Akan, Mole-Dagbani, Ewe and Ga-Dangme populations. Ethnic identity is not an exact language, literacy status, comfortable educational language or beneficiary count (Ghana Statistical Service, 2021a, 2021b, 2021c).
...
The identity rules are strict. “Central Nahuatl” receives no generic route; fifteen exact INALI varieties receive separate profiles and retain dated/shared-locality cautions. Quechua, Maya, Guaraní, Cree, Ojibwe, Inuktut and Sámi are not single educational languages. Tok Pisin, Solomon Islands Pijin and Bislama are distinct languages and do not cover Papua New Guinea tok ples, Solomon local languages or Vanuatu's other languages. Australian Indigenous languages, Pacific signed languages, exact diaspora cohorts and unswept Amazonian, Caribbean and circumpolar languages remain explicit possible-outranker/evidence obligations. The [regional packet](AMERICAS_PACIFIC_PACKAGE_NEEDS_20260904.md) and its machine-readable evidence preserve every number and boundary.
...
The population anchors cannot be added or assigned to modes. The report includes, among other contexts, Internet non-use, hearing-loss, vision-loss, adult-literacy and supply-side accessibility estimates. They overlap and do not measure screen-reader use, caption preference, sign-language identity, braille literacy, need for plain language or additional educational access. All mode-specific need, effect and cost values therefore remain unknown. Written input-token traffic is not the full cost of narration, captioning, sign performance, braille, tactile graphics, descriptions, compatibility work or device testing. Carrier standards likewise do not remove the need to localize text, metadata, pronunciation, descriptions, captions, narration and interface labels. The complete [accessibility package comparison](ACCESSIBILITY_PACKAGE_NEEDS_20260904.md) retains these boundaries.
...
The globally decisive portfolio is not yet complete. The current quantitative
surface now has a 100-row worldwide potential-reach table and a distinct
100-row fixed-package workload proxy, alongside the bounded ten-program hedge
and the nonordinal 100-profile research inventory. The two ranked tables answer
real but deliberately conditional questions; the profile inventory does not.
Required remaining work includes stage-specific addressable audiences;
item-level comparisons of independently usable subject provision; measured
incremental effects; standardized physical-compute scenarios; delivery and
accessibility costs; beneficiary overlap; global omission and sensitivity
tests; and comparisons against possible outrankers. Signed-language and
accessibility interventions, smaller communities, and vocational and research
opportunities remain substantive lanes rather than residual categories.
...
The requested Top 10 and Top 100 now exist in two explicitly conditional,
ordinal senses. The reach order compares population plausibly helped under one
common missing-data scenario. The fixed-package proxy divides that numerator
by the modeled token-event workload for one identical package. Neither is the
final package-specific funding order. Separately, the ten-program tranche is
one feasible hedge across large audiences, regional depth, displacement,
multiple scripts and practical subject needs, while the 100-profile set is a
nonordinal communication and investigation inventory. Its member numbers are
stable identifiers, not positions. The 169 current challengers and all entities
outside those displays remain possible outrankers. Accessibility formats and
interlanguages remain parallel, nonadditive design layers; all 209 origin and
178 host displacement contexts remain visible rather than being reduced to ten
examples.
...
The access commitment is not an empirical hurdle placed in front of learners.
This study treats inherited familiarity with academic language as an
institutionally distributed advantage and treats a source-faithful translation
into an audience-specified adult register as an eligible intervention whenever
any credible nonzero educational benefit is plausible. The literature can help
estimate the size and distribution of that benefit. It can show that a
particular source, audience, register or delivery design was ineffective or
counterproductive. It cannot convert family education, schooling history,
income, racialization, disability, measured attainment or register familiarity
into an innate-capacity judgment or a reason to withhold access.

The closest direct evidence includes Bälter et al. (2024), who analyzed 2,263
learners in Swedish and English versions of the same self-study programming
course, after excluding 212 participants who switched language after assignment.
Average scores were 7.24 versus 4.18 out of 42. Test initiation was 42.8% versus
29.2%; this means attempting at least one test question, not completing the course.
The more restricted active-student comparison was 16.9 versus 14.3 correct
answers. Section 7.10 calculates the consequences of these distinct endpoints
and the missing switcher outcomes. In a four-arm trial with 111 Scandinavian family
physicians, Gulbrandsen et al. (2002) found median recall of four of thirteen
facts after reading in the mother tongue and three after reading in English;
paper versus screen made no detected difference. These are unusually close to
the proposed language-edition contrast, but they concern highly educated
samples, short or bounded material and immediate or course-level outcomes. They
are anchors, not global multipliers.

Adult intralingual register experiments show both meaningful gains and strong
heterogeneity. Leroy et al. (2013) preserved source information while adding
definitions and familiar wording. In a 99-person experiment, correct
comprehension was 63% for the translated passages and 52% for the originals;
learning gains were 18% and 9%, while free recall did not differ. Martínez,
Mollica, and Gibson (2022) found a 5.8-percentage-point comprehension advantage
for equivalent-meaning ordinary English over legalese in short contracts.
Sayfi et al. (2024) found a 19.8-point advantage for a plain-language WHO
recommendation but only 3.9 points for a CDC text, whose confidence interval
included no effect. The large difference between two related sources is as
important as either headline number: source structure, audience and the exact
translation choices matter.

Null and adverse comparisons are therefore design information. They are not a
vote on whether a language or social group is capable of using educational
material. One bilingual physics comparison produced smaller gains than its
German-only counterpart, while a paper-reading interface added perceived ease
without a detected objective-comprehension gain. Such results require analysis
of terminology, cognitive load, prerequisites, layout, teaching context and
audience fit. They justify a different design or a smaller first deployment,
not permanent ineligibility.

Open access, offline delivery and instructional support also answer different
questions. Clinton and Khan's (2019) meta-analysis found essentially equal
average learning outcomes for open and commercial textbooks (*g*=.01, 95% CI
[-.16, .19]) and lower course-withdrawal odds for open-textbook conditions
(OR=.71, 95% CI [.56, .90]). That comparison supports substituting free books
for paid books without assuming an achievement penalty. It does not estimate
the effect of a usable book versus no book, or the effect of translation.
Likewise, making a file downloadable changes acquisition; making it reflowable,
screen-reader-native, printable and offline changes practical usability;
worked examples, retrieval practice, explanatory feedback, prerequisites and
self-regulation support change learning conditional on use.
...
For a passive release, those stages can be multiplied only when their
conditions and denominators are compatible. They must not be multiplied into a
study's intent-to-treat effect, which already includes uptake and nonuse. Nor
may a comprehension percentage be multiplied directly by a worldwide language
population without an exposure and use model.

The current evidence register therefore publishes scenarios rather than a
single promised effect. For a well-audited full-content adult-register
translation, the immediate-comprehension scenarios range from an adverse
-3-to-0 percentage points, through near-null 0-to-3 and central 3-to-10, to a
10-to-20-point high-mismatch case. For an actively used static package with
worked examples, retrieval, answers and explanatory feedback, the planning
range is 0.05–0.25 standard deviations over roughly a term or year. A merely
published file receives a 0–0.02 scenario. Preschool remains unmeasured rather
than being assigned zero. These are stress tests assembled from distinct
evidence classes, not confidence intervals, pooled global estimates or
eligibility thresholds.

No controlled study located so far evaluates a complete,
proposition-preserving mathematics curriculum translated into another language
or adult register. That is a reason to measure the first well-audited editions
carefully. It is not a reason to presume no benefit. The full 53-study evidence
register, 18 planning scenarios, source-to-scenario transport rules and
independent reproducibility checks are published in
`INTERVENTION_EFFECT_EVIDENCE_v1.md` and its machine-readable companions.

### 7.7 An actionable first tranche without pretending it is a world league table

A decision-maker may still need to act before every comparable cell is
measured. The current response is a ten-program measurement-and-production
tranche. It deliberately hedges across very large audiences, multilingual
systems, multiple scripts, displacement, offline access, foundational learning,
practical subjects and advanced study. Every program remains eligible
independently of the evidence gathered so far. The evidence selects its first
...
The shared measurement protocol uses a 100,000-source-token benchmark to learn
target-specific production, quality-assurance, accessibility and delivery costs.
That benchmark is not the first educational package and does not force every
language into a 100,000-token book. The first packages above differ because the
best available evidence points to different stages, subjects and delivery
problems. Complete courses and smaller additions are compared at the same
educational endpoint.

The wider 100-profile inventory is a communication set, not ranks 1–100. Its
member numbers are record identifiers. This differs from the newly published
100-row potential-reach order and 100-row fixed-package workload proxy, whose
columns are explicitly labelled as ordinal positions and whose quantities are
fully stated. All analytical entities outside the profile inventory remain
possible outrankers. The current target-specific partial order still contains
zero robust global dominance edges because no pair jointly binds compatible
audience, same-endpoint effect, measured physical compute and nonoverlapping
beneficiaries. The tranche is therefore an expert-constructed feasible hedge,
not the output of a falsely precise optimizer.

This is also why the list contains both very large educational languages and
regional, signed, displacement and shared-register lanes. A population-only
portfolio would suppress smaller high-severity and prestige-domain needs. A
scarcity-only portfolio would suppress hundreds of millions of people in large
languages. A documentation-readiness portfolio would reward communities that
are already easiest to measure. The conditional portfolio keeps those failure
modes visible and publishes the evidence that could change each slot. Its full
Top 10 rationales, nonordinal Top 100, accessibility overlays, displacement
portfolio, possible-outranker fields and deterministic checks are in
`CONDITIONAL_GLOBAL_PORTFOLIOS_v1.md` and its CSV/JSON companions.

### 7.8 From a population screen to a target-specific allocation frontier

The first target-specific frontier corrects a category mistake that the profile
catalogue can otherwise invite. A language profile is a search key. It is not an
intervention, a priority or a unit to which compute can be allocated. Compute is
allocated to a production-and-delivery plan that names the target audience,
educational endpoint, source package, language/register/script, formats,
delivery route, quality threshold and horizon.

Fifteen current routes contain a numeric need/context interval. Nine of them
share the declared secondary quantitative-catch-up endpoint and can therefore
enter a conditional comparison. Their point-scenario coefficients under equal
target-specific effect are Indonesian 2,305,230; Brazilian Portuguese
1,247,193; Filipino 951,470; Thai 246,160; Vietnamese 181,776; Standard Malay
146,227; Japanese 79,644; Central Khmer 47,980; and Ukrainian 44,546 context
members per million fresh-equivalent token events. These values are `N/C`, not
people known to have learned, and the denominator is workload traffic rather
than physical compute. The six other numeric routes remain visible but are not
cross-ranked against a different endpoint.

For every directional pair, the frontier publishes the exact effect-ratio
threshold at which the comparison reverses. With positive effects, route A
exceeds B when `q_A/q_B > (N_B/N_A)(C_A/C_B)`. This produces 72 directional
thresholds for the nine-route subset. A direct estimate can therefore replace
the equal-effect placeholder without rebuilding the comparison or silently
removing candidates. The independent review recomputes each threshold from the
source registers and verifies that no route receives a funding rank or exclusion
disposition.

Existing supply changes the proposed intervention, not the underlying need.
Indonesia has official Grade VII student books and separate teacher guides, so
the matched comparison is an autonomous diagnostic, worked-feedback,
remediation and offline companion versus a complete independent sequence—not a
claim that no Indonesian material exists. Brazil has a large represented
secondary-proficiency context, but the current evidence does not yet identify
whether language, prerequisites, pedagogy or delivery is the main constraint.
The Philippines has extensive K–12 and alternative-learning-system records,
fragmented access paths, a directly observed anonymous-download barrier and
unresolved allocation of burden among Filipino, English and exact local
languages. Japanese and Ukrainian have stage-matched package hypotheses, while
their most specific local frames do not equal the shared catch-up context. Thai,
Central Khmer and Standard Malay now have source-bound complete-course and
independent-companion proposals (section 7.16). Their existing worked lessons,
textbooks and answer routes inform what an additional package should supply;
they do not remove these populations from the worldwide comparison.
Vietnamese has a related secondary science-reading proposal rather than
the exact quantitative-catch-up package.

Delivery evidence also changes the intervention. Indonesia's cited 2024
national marginals report 72.78% internet use, 68.65% mobile-phone ownership and
18.52% household-computer ownership. Philippine sources report 67.3% internet
use among people aged ten and older and 48.8% household home internet, with
large regional differences. These are not learner × language × device
cross-tabs and are not multiplied into effect. They support offering
phone-readable, printable, download-once and offline forms together rather than
assuming continuous web access.

Package size must be judged against the same endpoint. At the pinned OpenStax
*Prealgebra 2e* source commit, the measured learning-content text contains
364,270 `cl100k_base` tokens across 75 modules. A declared six-chapter
quantitative selection contains 173,958 tokens, or 47.755% of the complete
source; a fractions-only unit contains 41,336 tokens, or 11.348%. With no common
setup cost and a genuinely common endpoint, the six-chapter package is more
efficient only if it delivers more than 47.755% of the full package's positive
yield. The threshold rises to 52.505% when common setup is one tenth of full
variable production cost and to 73.878% when it equals that cost. A
fractions-only unit cannot be credited with the complete catch-up endpoint; its
lower arithmetic threshold applies only to an explicitly narrow fractions
outcome.

The immediate empirical comparison is therefore not “which language wins?” It
is: for Indonesian, Brazilian Portuguese and Filipino, specify one identical
quantitative endpoint and horizon; compare a bounded prerequisite-and-feedback
companion with a complete autonomous sequence; model passive publication,
active static use, offline use and supported use separately; and measure the
full accepted-package production work on a declared reference system. That
experiment is the shortest route from the current population screen to a
defensible compute-allocation frontier. In parallel, unresolved high-reach and
high-severity routes remain explicit possible outrankers so evidence scarcity
cannot turn into exclusion.
...
### 7.11 Shared editions under a finite budget

A new [two-page supplement](https://kokunoyumeto.github.io/allocating-ai-translation-compute-for-educational-access/research/shared-editions-20260908/10_Shared_Editions_Budget_Study_20260908.html) compares a dual-script Hindi-Urdu mathematical register with the best affordable mix of native-language editions. It retains the same screening inputs and gives every portfolio the same one-time source-preparation saving. The four declared cost decompositions produce twenty bounded optimization results, all independently checked for exact selected-cost and coefficient arithmetic.

In the low-preparation scenario, the native mix is Chinese, Hindi, Spanish, Portuguese and Indonesian; buying the bridge leaves room for Chinese, Spanish and Arabic. With no modeled pair overlap, bridge benefit above 78.29% of the normalized combined native opportunity would improve this particular portfolio. With fully nested pair audiences, 120.54% would be required. A high-shared-preparation scenario has an interior minimum of 87.41%, so its endpoints alone do not describe the full sensitivity. These are conditional benefit thresholds, not observed comprehension rates, new-learner counts or physical-compute measurements.

This is an allocation test rather than blanket credit or rejection for an interlanguage. It compares against the best affordable alternatives, not only against translating both native editions. Shared opportunity, script access, educational yield, other multilingual overlaps and native-plus-bridge hybrids remain explicit unresolved parameters or alternatives. None becomes zero need. The local source-bound calculation, selected portfolios and independent audits are in `structured/HINDI_URDU_BUDGET_ENTRY_FRONTIER_v1.json` and the supplement source archive. The full worldwide scope remains active.

### 7.12 What a focused package can afford to add

The next allocation comparison asks how much work can be spent making a focused package useful before a complete-book edition becomes more efficient. The [four-page focused-package study](https://kokunoyumeto.github.io/allocating-ai-translation-compute-for-educational-access/research/focused-packages-20260908/14_Focused_Learning_Packages_20260908.html) combines actual OpenStax source structure, four direct instructional studies and exact conditional workload calculations. It does not assume that the existing source has no exercises or answers, nor that selecting fewer pages preserves the full educational benefit.

The pinned fractions source contains 108 example containers, 216 Try It notes, 19 readiness notes and 1,047 exercise nodes; 696 exercises contain a solution element. Categories are nested, not additive. These are structural counts, not a guarantee that every solution is a full explanation. Its 367 image-reference instances and links to eight outside modules identify further production work. Adding those eight directly linked modules once increases the extracted source from 41,336 to 87,655 tokens, compared with 364,270 for the full collection. This one-hop scenario is not a complete prerequisite closure or a claim that every linked module is necessary.

Let the full package cost F + V, with positive variable workload V and common setup F = fV. Let the focused package cost F + rV + E, where r is its declared source-length workload ratio and E is additional work. If alpha(E) is its final additional educational benefit relative to the full package's positive additional benefit, on the same cohort, learning outcome, alternative provision and time horizon, focused production is more efficient precisely when `E/V < alpha(E)*(1+f)-f-r`. Equality is a tie. When extra work changes benefit, this is a condition on the finished design, not a spending cap determined from the unimproved design.

At half the full package's benefit and setup equal to 10% of full variable work, the fractions-only base can absorb extra work of up to 33.65% of V before break-even. Including all eight directly linked modules leaves 20.94% of V for further work. Six chapters at half-benefit are already less efficient in this setup scenario; at three-quarters benefit they can absorb 24.74% of V. Ratios above one are also tested: a focused package can outperform a full book on a narrow outcome, although source length alone does not establish that result. All these benefit and setup values are declared scenarios, not fitted empirical parameters.

In the common Indonesian workload example, full-source variable production is 4.891 million token-event workload units; 10% setup adds 0.489 million. The fractions-only base is 0.555 million, leaving a 1.646 million extra-work break-even allowance at half-benefit. These units do not measure physical compute, cost in money, subscription usage or achieved learning. Chinese, Hindi, Portuguese and Filipino conversions use the same rule; all 775 inherited profile IDs remain present. No local programme accomplishment changes population need or eligibility.

The instructional literature informs concrete design rather than supplying an invented common effect. Sweller and Cooper (1985) support studying example/problem pairs in a supported short-session setting. Booth et al. (2015) found differing responses to algebra-example worksheets, rather than a significant universal effect in the larger experiment. McGuire et al. (2017) found no significant score advantage for richer fraction feedback in its selected analysis and a lower completion rate in the picture-hint group. Pitchford (2015) found useful gains from a supported Chichewa tablet package, not an isolated translation contrast or unsupported home-study effect. The supplement preserves populations, comparators, periods, selection limits and exact primary-source locators.

The practical recommendation is to compare complete-book and focused, independently usable designs on an explicit learning gap. Retain available examples, practice and answer routes; price the missing supports and delivery work; keep faithful translation separate from added pedagogy. The full book's other outcomes still count when the objective is a whole course. At global scale these alternatives must enter the fixed-budget portfolio together with audience overlap and reusable costs. Higher average benefit per workload alone does not prove which indivisible combination maximizes total benefit. The worldwide rankings and all unresolved alternatives remain part of the study.
...
### 7.14 Practical business education: production and delivery are different decisions

For adult micro-retailers, a practical business-records and decisions package is a specific vocational candidate alongside, rather than a substitute for, comprehensive accounting and corporate-finance courses. Proposed content includes explained cash records, customer-credit records, stock decisions and supplier comparisons, with worked examples, answers, reusable blank records and locally appropriate audio and text. An independently usable download should not require an ongoing teacher, account or connection. Its effect remains a disclosed scenario rather than an effect borrowed unchanged from supported instruction.

Two experiments sharpen this resource choice. Drexler, Fischer, and Schoar (2014) compared supported practical-rules and conventional-accounting courses among 1,193 loan clients in Santo Domingo. Scheduled contact time was 15 and 18 hours respectively. Both arms shared three introductory sessions and included instructors, handouts and homework. The practical course improved the business-practices index against control, but its full-sample index contrast with accounting was not statistically established (equality p = .195). Cash separation specifically favored the practical arm (p = .014). Outcomes among observed business owners and attrition-sensitive revenue findings do not identify mastery, profit gains or the effect of an unattended translation. The article does not explicitly name its delivery language.

Cole, Joshi, and Schoar (2024) tested short voice messages plus an orientation among retail microfinance clients in India and the Philippines. Hiligaynon is the stated Philippine eligibility language; Hindi and Kannada recordings are explicitly costed. Urdu was also an Indian eligibility language, but a separate Urdu recording and language-specific effect are not established by the cost paragraph. Practice-index effects support this kind of vocational offer without establishing higher regular-week profits or direct conceptual comprehension. Exact language, occupation and delivery matter; the national label Filipino would hide the Hiligaynon opportunity.

The study reports 73 minutes of Hindi audio, 85 of Kannada and 81 for the Philippine route. Historical complete-listening airtime alone cost USD 2.04, USD 2.38 and USD 14.99 respectively. These are study-era marginal airtime figures, not present quotations, full delivery costs or AI compute. At an illustrative 24 kbps, the audio payloads would be 13.14, 15.30 and 14.58 MB before headers and accompanying files. This makes an offline voice/text route concrete without pretending its uptake or effectiveness has been measured by the voice-call experiment.

The allocation objective must therefore price source preparation, translation and format production separately from per-recipient distribution or support. Compare additional understanding on the same endpoint over each cohort's currently usable alternative. An offer-level effect already incorporates uptake. Missing occupational audience counts and direct comprehension effects remain explicit parameters; neither the trial sample nor the entire language population is the vocational beneficiary count. The worldwide language baselines and rankings remain unchanged.

For example, if a download reaches five times as many distinct relevant learners as a supported route, it needs to retain more than 20% of the latter's offer-level benefit to improve total benefit, once both options fit the stated budgets. That is a conditional threshold, not a measured fivefold reach advantage. Where support is offered to people already reached by download, count only its extra benefit for those people. A scenario with download benefit 0.40, supported benefit 1.00 and support reaching half the same cohort yields 0.70, not the naive sum 0.90. Complementary outcomes require a different model.

The immediate resource recommendation is practical, independently usable business education for exact-language micro-retail audiences, with deeper accounting available for the distinct conceptual objective. Hiligaynon, Hindi and Kannada are specific source-supported leads; Urdu and other communities remain eligible with their evidence status disclosed. This is not a new worldwide language order. It adds a vocational option and a separate delivery constraint to the allocation problem. VOCATIONAL_PACKAGE_ALLOCATION_v1.md and its structured calculation preserve the source evidence, original design inference, exact thresholds and remaining uncertainties.

### 7.15 How audience evidence can change a compute allocation

Language populations are starting points, not automatically the audiences for every educational resource. A retail-accounting course, a secondary algebra course and a research monograph serve different immediate and future learners. The worldwide language-community and academic-English-access baseline remains visible; package-specific evidence adds to it rather than removing communities from consideration.

When only separate language and occupational totals are available, their intersection is not identified. In a common population of N people, with A meeting the language condition and B the resource-audience condition, the joint count X satisfies `max(0,A+B-N) <= X <= min(A,B)`. These bounds follow from nonnegative cells in the two-by-two population table. `A*B/N` is instead an independence scenario. Disjoint, exhaustive regional detail can tighten the bounds without assuming independence.

An exact synthetic example shows the allocation consequence. A population of 100 contains 40 people meeting a language condition and 30 meeting an occupational condition, giving aggregate bounds of 0 to 30 and an independence value of 12. Two regions of 50 have respective language counts 35 and 5 and occupational counts 20 and 10. Their joint bounds sum to 5 to 25; within-region independence gives 15. Both endpoints are feasible. None of these illustrative counts represents a named community.

Give the targeted resource cost 2 and this uncertain audience; give an alternative resource cost 2 and audience 20, and a third resource cost 1 and audience 6. Assume distinct beneficiaries and a common positive reach-times-additional-benefit factor, normalized to one. With budget 3, the optimal choice changes from the alternative plus third resource at joint audience 5 to the targeted plus third resource at joint audience 25. Their additional-benefit totals are respectively 26 and 31. At joint audience 20 the combinations tie. These are arbitrary cost and normalized benefit units, not measured compute or observed learning. Missing joint evidence therefore identifies a possible allocation reversal, not an ineligible language. A zero lower bound is not zero need.

Primary survey documentation supplies routes to better estimates. IHDS-II records respondent-associated mother tongue and business decision-maker IDs that can be linked to its person roster; this is not individual academic-language proficiency. The Philippine 2020 census dictionary similarly links household home-language and work-industry concepts. Neither access check obtained analysis-ready joint records, and no national entrepreneur or learner count is claimed. These historical routes also require explicit updating assumptions before any present-day use (NCAER & University of Maryland, n.d.; Desai & Vanneman, 2018; Philippine Statistics Authority, 2024a, 2024b).

Weights need the same care as audience definitions. An individual cohort share `sum(w*I)/sum(w)` is unchanged when every survey weight is multiplied by a constant, whereas the numerator changes. Thus an absolute population count requires a documented expansion scale or a matched population calibration. Household-size weights must not be applied again to each person. Repeated business slots must not count the same learner repeatedly; missing language or work information remains unresolved rather than false. The IHDS guidance supports individual cross-sectional WT and documents household-size multiplication in INDWT (Lazaroff, n.d.; India Human Development Survey, n.d.).

The accompanying calculation implements exact compatible-count bounds, stratified alternatives, budget selections and weighted-share checks. Its 26 checks cover 819 small marginal-count combinations and 3,025 pairs of strata; these validate the stated arithmetic, not real population estimates or survey-key and weight provenance. Fractional independence scenarios are distinguished from actual integer population tables. The decision rule for further research is to retain an explicit reference scenario and possible winning alternatives while replacing assumptions with comparable joint evidence. This applies to language, academic register, educational stage, occupation, device access and interlanguage overlap. Additional learning and standardized physical compute remain distinct quantities to estimate. The full method and source passages are recorded in `AUDIENCE_JOINT_ALLOCATION_v1.md` and `structured/AUDIENCE_JOINT_ALLOCATION_v1.json`.

### 7.16 What secondary mathematics packages can add in Thai Malay and Khmer

Primary learning materials support specific additions for all three profiles. The useful comparison is a complete independent catch-up course against a companion that makes existing lessons easier to study autonomously. Neither should be justified by assuming that these communities have no mathematics textbooks or worked answers. Existing provision helps identify the intervention; it does not establish that everybody can use it, or lower a community's worldwide need estimate.

For Thai, the IPST lower-secondary guide specifies rational numbers, fractions and decimals, ratios and percentages, and one-variable equations at Mathayom 1. The DLTV equation lesson supplies worked solutions and checks: PDF pages 5-6 of the linked slides solve a consecutive-even-number problem and verify the result against its conditions. The linked activity sheet also asks learners to invent a number trick and, in a separate activity, work in groups to create, exchange, solve and check a word problem. These are substantial existing resources. A solo companion should add a diagnostic route through prerequisites, worked exemplars for open-ended tasks, and criteria for checking a learner's own problem and solution. It should preserve the mathematics while supplying feedback that a lone learner cannot obtain from a blank worksheet. The group requirement applies explicitly to Activity 6, not automatically to every activity or lesson (IPST, n.d., PDF pp. 9-11, 13; DLTV, 2025; DLTV, n.d.-a, PDF pp. 5-6; DLTV, n.d.-b, pp. 1-2).

For Standard Malay in Malaysia, the district education site links Form 1-3 Malay textbooks separately from English DLP editions. The actual books contain worked examples, practice and answer sections. In Form 1, the algebraic-expression example and practice on printed page 108 have corresponding answers on page 312; Form 2 and Form 3 also supply worked algebra and answer routes. An additional package should therefore target independent explanation and practice, rather than promise the first answer key. Malaysia's TIMSS 2023 item analysis gives a concrete skill lead: 12.5% answered the sampled fraction-subtraction item correctly, and 22.8% answered the signed-value substitution item correctly. The report discusses denominator reasoning and handling negative values. These are item results for assessed students, not percentages of all Malay speakers and not evidence that language caused the errors. They support an original diagnostic-and-feedback sequence on fractions, signs, expressions and elementary equations (PPD Batu Pahat, n.d.; Ooi et al., 2016, pp. 108, 312; Bahariah et al., 2017, pp. 20-21, 294-295; Chiu et al., 2018, pp. 23, 252; Kementerian Pendidikan Malaysia, n.d., pp. 56-57, 78-79).

For school-register Khmer, a ministry-domain school library links a Grade 9 textbook and a separately authored solutions book. The textbook explains proportions and unit conversion on printed pages 17-18 and works through linear equations on page 41. Sample pages of the separate solutions file show multi-step numerical and algebraic work. In the downloaded textbook, all 241 pages returned empty extracted text and no PDF structure tree was present; visually inspected pages were scans. This supports a searchable Khmer Unicode and semantic-mathematics edition with an adaptable layout, not a claim that Khmer mathematical content is absent. A companion can connect prerequisite explanations, a problem, worked reasoning and answer feedback in the same offline reading path. The retained solution book is attributed by the school library, not asserted to be ministry-authored (School E-Library, n.d.; Ministry of Education, Youth and Sport, 2020 printing, pp. 17-18, 41; Hai Pahin, n.d., PDF p. 11).

Download and display should be costed explicitly. The Malaysian Form 1 file is 104.7 MB; the two Khmer mathematics files total 51.0 MB, using decimal megabytes. These observed sizes motivate small topic downloads and reflowable offline HTML as options alongside a full bundle and print PDF. They do not prove that every learner is unable to download or read the existing files. Thai static PDFs were directly downloadable; this inspection did not test the video player or real-phone usability. For all three profiles, the proposed language and script remain explicit: Thai in Thai script, Malaysian Standard Malay in Latin script, and school-register Central Khmer in Khmer script. None stands for every language used in its country.

The package proposals use original or appropriately open-licensed content aligned to the inspected curricular and terminological sources. Downloadability of the sampled books and assessment report is not treated as permission to reproduce them. The source studies preserve exact file identities, edition qualifications and consulted pages. Their present allocation contribution is to specify what to build and price. Relative learning effects remain disclosed assumptions in the package comparisons, and complete-course, companion and accessible-format options all remain eligible.

### 7.17 Prerequisites can change which package is worth translating

A short course is not automatically a low-cost way to achieve the same learning outcome. Its apparent saving can depend on learners already knowing material that has been omitted. This matters particularly for an independent-study edition: a reference to an unavailable lesson is not the same as providing the explanation. The useful production unit is a learning path with the necessary support, not an arbitrary number of translated pages.

The pinned OpenStax *Prealgebra 2e* source makes this problem concrete. The earlier six-chapter proxy contains 36 modules and 173,958 content-text tokens. Adding chapters 2, 5 and 7 supplies algebraic language, decimal/rate work and properties of real numbers, bringing the selection to 56 modules and 275,407 tokens. A further lesson, *Solve a Formula for a Specific Variable*, is explicitly referenced by the readiness questions for graphing linear equations and graphing with intercepts. Including that lesson yields 57 modules and 283,936 tokens. The full source collection contains 364,270 tokens under the same content extraction and tokenization convention (OpenStax, 2026a, source modules m81329, m81345 and m81350; extraction and source identities are provided in the accompanying source-coverage report).

These additions materially change the economic comparison even before estimating learning effects. The table holds the target language and workload per source token constant. Each companion has an additional allowance equal to 5% of full-book variable work for diagnostic tasks, feedback and integration; the complete-book option is the comparison base. The allowance is a disclosed scenario, not a measured production constant. Shared preparation is excluded from this table and included separately in the finite-budget calculation.

| Production option | Content-text tokens | Variable work relative to the complete book | Same-outcome benefit needed to match full-book benefit per unit of variable work |
|---|---:|---:|---:|
| Six-chapter companion proxy | 173,958 | 52.76% | 52.76% |
| Nine-chapter intermediate companion | 275,407 | 80.61% | 80.61% |
| Nine chapters plus formula rearrangement | 283,936 | 82.95% | 82.95% |
| Complete source collection | 364,270 | 100.00% | 100.00% |

Thus the six-chapter companion could match the complete book's benefit per unit of variable work while retaining about 53% of its relevant benefit. Once the larger prerequisite selection is supplied, that threshold rises to about 83%. The remaining variable-work saving is about 17%, not almost one half. This does not measure how well either package teaches, and it does not establish that the complete book is best: a carefully designed smaller package may still retain most of the benefit for its intended endpoint. It shows why an allocation model must price the prerequisites it expects a learner to use.

The source check found 28 explicit readiness links to omitted modules in the original selection, two in the nine-chapter intermediate, and none in the 57-module selection. All 132 explicit readiness-link occurrences in the final selection resolve to included source material. Seven other cross-references still point outside it and are recorded separately; they were not silently counted as readiness instructions.

The fixed-budget calculation then prices the two production alternatives for the same nine previously defined secondary-education contexts. It enumerates every possible choice of no package, companion or complete book for each context. Across 24 scenarios, adding the prerequisite material changes the selected packages in nine cases. The country survey contexts, benefit hypotheses and workload rates stay unchanged, so these changes arise from package scope rather than a reassessment of any community's need.

One example shows why neither "translate the shortest package everywhere" nor "translate the complete book everywhere" is a general allocation rule. With a budget of 10 million standardized workload units, shared source preparation charged at two units per source token, and a companion assumed to achieve half the complete book's relevant benefit, the best combination in this nine-context scenario is a complete Indonesian edition and a Brazilian Portuguese companion. It uses about 9.64 million units. Complete editions for those two contexts would require about 10.47 million and do not fit. The mixed choice produces 26.82% more modeled benefit than the best feasible all-companion or all-complete policy under those assumptions. This percentage is an exact comparison within the stated scenario, not a measured gain in learning or a worldwide recommendation to prefer these two languages. The scenario compares the same outcome with the same unmeasured full-package benefit per learner; it does not assert different learning ability by nationality.

The 57-module selection is a deliberately conservative source alternative, not a minimal curriculum or a claim that every pedagogical prerequisite has been identified. Whole chapters can include unnecessary material, while a hyperlink inventory cannot establish that every concept needed for independent understanding is explained. Production should retain necessary support, remove genuinely unnecessary duplication only with content-based justification, and preserve exercises, answers and explanatory feedback. The old six-chapter calculation remains available as a separate scenario rather than being silently rewritten.

The comparison concerns standardized token-workload scenarios, not observed accelerator time, energy or monetary cost. It changes the package-cost assumptions used for the same secondary quantitative endpoint; it does not change any language's worldwide population baseline or turn this nine-context study into the global Top 10. Existing full-course, companion, register-translation and accessible-format alternatives remain eligible throughout the broader allocation study.

## References used in this draft

Institute for the Promotion of Teaching Science and Technology. (n.d.). *Lower-secondary mathematics curriculum-use guide for the 2017 revision* [Thai]. https://www.scimath.org/e-books/8380/8380.pdf

Distance Learning Foundation. (2025, December 1). *M1: Solving one-variable linear-equation problems (3)* [Thai lesson, episode 102574]. https://dltv.ac.th/teachplan/episode/102574

Distance Learning Foundation. (n.d.-a). *Teaching slides linked from episode 102574* [Thai PDF]. https://dltv.ac.th/utils/files/download/205531

Distance Learning Foundation. (n.d.-b). *Activities 5 and 6: M1 one-variable equations* [Thai PDF]. https://dltv.ac.th/utils/files/download/205532

Pejabat Pendidikan Daerah Batu Pahat. (n.d.). *Matematik: Buku teks digital sekolah menengah*. https://sites.google.com/moe-dl.edu.my/ppd-batu-pahat-sains-matematik/buku-teks-digital-bidang-sains-matematik/buku-teks-sekolah-menengah/matematik

Ooi, S. H., Yong, K. Y., & Ng, S. H. (2016). *Matematik Tingkatan 1*. Penerbitan Pelangi for Kementerian Pendidikan Malaysia. https://drive.google.com/file/d/1uGBdQeSNrRbH1caQ33GM7ccjfVeDm-8C/view

Bahariah binti Hj. Baharam, Baharizah binti Hj. Baharam, Nurul Jannah binti Ahmad, Nurazreen binti Mohd Tahir, & Mohd Nazri bin Mohd Hanafiah. (2017). *Matematik Tingkatan 2*. Rimbunan Ilmu for Kementerian Pendidikan Malaysia. https://drive.google.com/file/d/1G9leLwWhwj6sZEuHmiMwMlRMXMxBLE3_/view

Tertiary Education Trust Fund. (n.d.). *TERAS services*. Retrieved September 5, 2026, from https://teras.tetfund.gov.ng/services

UNICEF Nigeria. (2023). *Nigeria education fact sheets: Analyses for learning and equity using MICS data*. https://www.unicef.org/nigeria/media/9211/file/Nigeria%20Education%20Fact%20Sheets.pdf

Universal Basic Education Commission, & UNICEF. (2025). *National Assessment of Learning Achievement in Basic Education: NALABE 2022 report*. https://www.unicef.org/nigeria/reports/national-assessment-learning-achievement-basic-education

World Bank. (2026a). *World Development Indicators: Nigeria* [Data set]. Retrieved September 5, 2026, from https://data.worldbank.org/country/nigeria

World Bank. (2021). *Loud and clear: Effective language of instruction policies for learning*. https://www.worldbank.org/en/topic/education/publication/loud-and-clear-effective-language-of-instruction-policies-for-learning

World Bank. (2026b). *World Development Indicators: Pakistan* [Data set]. Retrieved September 5, 2026, from https://data.worldbank.org/country/pakistan

World Bank. (2026c). *World Development Indicators: Ukraine* [Data set]. Retrieved September 5, 2026, from https://data.worldbank.org/country/ukraine

World Bank. (2026d). *World Development Indicators: Russian Federation* [Data set]. Retrieved September 5, 2026, from https://data.worldbank.org/country/russian-federation

OpenStax. (2026a). *Prealgebra 2e source collection* (Git commit `38cae454e644abf9f0a623e876994553881597c9`) [CNXML source]. GitHub. https://github.com/openstax/osbooks-prealgebra-bundle/tree/38cae454e644abf9f0a623e876994553881597c9

OpenStax. (2026b). *Introductory Statistics 2e source collection* (Git commit `1f6a35825395bb4aa2834cf1eca37512655f920c`) [CNXML source]. GitHub. https://github.com/openstax/osbooks-introductory-statistics-bundle/tree/1f6a35825395bb4aa2834cf1eca37512655f920c

OpenStax. (2026c). *Biology 2e source collection* (Git commit `63f8b6f8d129dd1582989bb755011e9a6d523471`) [CNXML source]. GitHub. https://github.com/openstax/osbooks-biology-bundle/tree/63f8b6f8d129dd1582989bb755011e9a6d523471

OpenStax. (2026d). *Chemistry 2e source collection* (Git commit `3be4b60ff501f29a445f0cacf003e5f5cc16244d`) [CNXML source]. GitHub. [Source collection at the cited commit](https://github.com/openstax/osbooks-chemistry-bundle/tree/3be4b60ff501f29a445f0cacf003e5f5cc16244d).

OpenStax. (2026e). *College Physics 2e source collection* (Git commit `fd1b25dfd5d8c6580c6e2b2b34a19e29cc69ada9`) [CNXML source]. GitHub. https://github.com/openstax/osbooks-college-physics-bundle/tree/fd1b25dfd5d8c6580c6e2b2b34a19e29cc69ada9

OpenStax. (2026f). *Principles of Management source collection* (Git commit `f2b749648a176dbe114dbc4e35ec20c030be4713`) [CNXML source]. GitHub. https://github.com/openstax/osbooks-principles-of-management-bundle/tree/f2b749648a176dbe114dbc4e35ec20c030be4713

OpenStax. (2026g). *Principles of Finance 2e source collection* (Git commit `e3e5135ae36e21282d7afb4cccd5ce09f421d830`) [CNXML source]. GitHub. https://github.com/openstax/osbooks-principles-finance/tree/e3e5135ae36e21282d7afb4cccd5ce09f421d830

OpenStax. (2026h). *Anatomy and Physiology 2e source collection* (Git commit `716383a4c6c16037b14d75a156c65145e75e895e`) [CNXML source]. GitHub. https://github.com/openstax/osbooks-anatomy-physiology/tree/716383a4c6c16037b14d75a156c65145e75e895e

OpenStax. (2026i). *Introduction to Python Programming source collection* (Git commit `d215dd3b99c33db855c49315152ebd5fbadc8534`) [CNXML source]. GitHub. https://github.com/openstax/osbooks-introduction-python-programming/tree/d215dd3b99c33db855c49315152ebd5fbadc8534

Houston Independent School District. (2022). *Immigrant student program evaluation report 2021–2022*. https://www.houstonisd.org/fs/resource-manager/view/a76fe050-7086-4b96-9bdc-540738502595

Houston Independent School District. (n.d.-a). *Multilingual program models*. Retrieved September 5, 2026, from https://www.houstonisd.org/schools-academics/academics/multilingual/multilingual-program-models

Houston Independent School District. (n.d.-b). *Testing & identification*. Retrieved September 5, 2026, from https://www.houstonisd.org/schools-academics/academics/multilingual/testing-identification

Texas Education Agency. (n.d.-a). *Bluebonnet Learning: K–5 math and secondary mathematics*. Retrieved September 5, 2026, from https://tea.texas.gov/curriculum-and-instruction/instructional-materials/bluebonnet-learning/bluebonnet-learning-k-5-math-and-secondary-mathematics

Texas Education Agency. (n.d.-b). *Texas Essential Knowledge and Skills in Spanish*. Retrieved September 5, 2026, from https://tea.texas.gov/laws-and-rules/state-board-education/teks/texas-essential-knowledge-and-skills-spanish

Wharton K-8 Dual Language Academy. (n.d.). *Academic offerings*. Retrieved September 5, 2026, from https://wharton.houstonisd.org/academics-programs/academic-offerings
...
Badan Pemeriksa Keuangan. (2003). *Undang-Undang Republik Indonesia Nomor 20 Tahun 2003 tentang Sistem Pendidikan Nasional*. https://peraturan.bpk.go.id/Download/32160/UU%20Nomor%2020%20Tahun%202003.pdf

Badan Pusat Statistik. (2025). *Telecommunication statistics in Indonesia 2024*. https://www.bps.go.id/id/publication/2025/08/29/beaa2be400eda6ce6c636ef8/telecommunication-statistics-in-indonesia-2024.html

Kementerian Pendidikan Dasar dan Menengah. (n.d.). *Sistem Informasi Perbukuan Indonesia: Katalog buku*. Retrieved September 5, 2026, from https://buku.kemendikdasmen.go.id/katalog

Pusat Standar dan Kebijakan Pendidikan. (2023). *Optimising the use of local languages in early grades* (Policy Brief No. 1). https://pskp.kemendikdasmen.go.id/assets_front/images/produk/1-gtk/kebijakan/Policy_Brief-Optimising_the_Use_of_Local_Languages_in_Early_Grades_FIN_%281%29.pdf
...
Bangladesh Bureau of Educational Information and Statistics. (2026). *Bangladesh education statistics 2024*. https://banbeis.gov.bd/pages/static-pages/6a338d7fcb6cb358ff185b6c

Bangladesh Bureau of Statistics. (2026). *ICT access and use by households and individuals survey 2024–25*. https://bbs.gov.bd/pages/files/6a609418b859b8767ff841e0

Islam, M. T., Hasan, M. K., & Akhand, M. M. (2025). Challenges in English-medium instruction classes among engineering major students at Bangladeshi private universities. *Studies in English Language and Education, 12*(1), 285–300. https://doi.org/10.24815/siele.v12i1.38407
...
OECD. (2023b). *PISA 2022 statistical tables: Annex A2 and Chapter 3*. [Annex A2 statistical tables](https://stat.link/hpg9nd) and [Chapter 3 statistical tables](https://stat.link/znxau8).

OECD. (2023c). *PISA 2022 results: Ukrainian regions (18 of 27) country note*. https://www.oecd.org/content/dam/oecd/en/publications/reports/2023/11/pisa-2022-results-volume-i-and-ii-country-notes_2fca04b9/ukrainian-regions-18-of-27_de3ad3b1/78043794-en.pdf

Eurostat. (2026, August 11). *4.41 million under temporary protection in June 2026*. https://ec.europa.eu/eurostat/web/products-eurostat-news/w/ddn-20260811-1

HSE University Institute for Statistical Studies and Economics of Knowledge. (2025). *Education in figures: 2025*. https://www.hse.ru/en/primarydata/oc2025

Kyiv International Institute of Sociology. (2026, June 15). *Dynamics of attitude to the status of the Russian language in Ukraine during 1997–2026*. https://www.kiis.com.ua/?cat=reports&id=1619&lang=eng&page=1

National Audit Office of Estonia. (2023, June 1). *Estonian language training in a Russian-language basic school*. https://www.riigikontroll.ee/en/audits/estonian-language-training-russian-language-basic-school

State Statistics Committee of Ukraine. (2001). *All-Ukrainian population census 2001: Results*. https://2001.ukrcensus.gov.ua/d/mono_eng.pdf

UNESCO, & United Nations Ukraine. (2024, March 11). *Analysis of war damage to the Ukrainian science sector and its consequences*. https://ukraine.un.org/en/262987-analysis-war-damage-ukrainian-science-sector-and-its-consequences
...
ASER Centre. (2025). *ASER 2024 national findings*. https://asercentre.org/wp-content/uploads/2022/12/ASER-2024-National-findings.pdf

Department of Higher Education, Ministry of Education, Government of India. (2026). *All India Survey on Higher Education 2023–24: Final report*. https://cdnbbsr.s3waas.gov.in/s392049debbe566ca5782a3045cf300a3c/uploads/2026/07/202607131602421770.pdf

Department of School Education & Literacy, Ministry of Education, Government of India. (2025). *UDISE+ report 2024–25: NEP structure*. https://www.dsel-education.gov.in/statistics

DIKSHA. (n.d.). *Features and supported languages*. Retrieved September 5, 2026, from https://diksha.gov.in/help/faqs/user/features.html

Indian Sign Language Research and Training Centre. (n.d.). *Frequently asked questions: ISL Dictionary*. Retrieved September 5, 2026, from https://islrtc.nic.in/faq/

National Council of Educational Research and Training. (n.d.). *Indian-language primers*. Retrieved September 5, 2026, from https://ncert.nic.in/primers.php

National Statistical Office. (2025). *Comprehensive Modular Survey: Telecom, 2025* (NSS Report No. 593). https://mospi.gov.in/sites/default/files/publication_reports/CMST_report_m.pdf

NPTEL. (n.d.). *Translation programme and translated books*. https://nptel.ac.in/translation

Agency for Cultural Affairs, Japan. (n.d.). *Languages and dialects in critical situations*. Retrieved September 5, 2026, from https://www.bunka.go.jp/seisaku/kokugo_nihongo/kokugo_shisaku/kikigengo/

Cabinet Office, Japan. (2025). *Promotion of measures concerning sign language*. https://www8.cao.go.jp/shougai/suishin/jsl.html

Japan Science and Technology Agency. (n.d.). *J-STAGE overview*. Retrieved September 5, 2026, from https://www.jstage.jst.go.jp/static/pages/JstageOverview/-char/en

Ministry of Education, Culture, Sports, Science and Technology, Japan. (2024a). *Statistical abstract 2024: School education and science-and-technology tables* [Data set]. https://www.mext.go.jp/b_menu/toukei/002/002b/1417059_00009.htm

Ministry of Education, Culture, Sports, Science and Technology, Japan. (2024b). *Results of the 2023 survey on the acceptance of students requiring Japanese-language instruction*. https://www.mext.go.jp/content/20240808-mxt_kyokoku-000037366_4.pdf

Ministry of Education, Culture, Sports, Science and Technology, Japan. (2024c). *Japan's student performance for PISA 2022*. https://www.mext.go.jp/content/20240524-mxt_chousa02-100002205_2.pdf

Ministry of Education, Culture, Sports, Science and Technology, Japan. (2024d). *The fourth basic plan for the promotion of education* [English translation]. https://www.mext.go.jp/content/20240228-soseisk02-100000597_09.pdf

Coordination for the Improvement of Higher Education Personnel. (2025). *Metadata for stricto sensu postgraduate students in Brazil, 2021–2024*. https://dadosabertos.capes.gov.br/dataset/c6bd4dca-a0fb-499a-9f7f-df0740563333/resource/70633e5c-5294-4cd4-abea-79341c63e406/download/metadados_discentes_pos_graduacao_2021_2024.pdf

Federal Senate of Brazil. (2021). *Law No. 14,191 of 3 August 2021: Bilingual education for deaf people*. https://legis.senado.gov.br/norma/34639654

Brazilian Institute of Geography and Statistics. (2025a). *2022 Demographic Census: Indigenous ethnicities and languages—Supplementary Table 21* [Data set]. https://ftp.ibge.gov.br/Censos/Censo_Demografico_2022/Etnias_e_Linguas_Indigenas_principais_caracteristicas_sociodemograficas_Resultados_do_universo/Tabelas_complementares/xlsx/Tabela_complementar_21.xlsx

Brazilian Institute of Geography and Statistics. (2025b). *2022 Demographic Census: Indigenous ethnicities and languages—Selected Tables 7–9* [Data set]. https://ftp.ibge.gov.br/Censos/Censo_Demografico_2022/Etnias_e_Linguas_Indigenas_principais_caracteristicas_sociodemograficas_Resultados_do_universo/Tabelas_selecionadas/xlsx/

Brazilian Institute of Geography and Statistics. (2025c). *2022 Demographic Census: Indigenous people—Literacy*. https://biblioteca.ibge.gov.br/visualizacao/periodicos/3111/cd_2022_indigenas_alfabetizacao.pdf

Instituto Nacional de Estudos e Pesquisas Educacionais Anísio Teixeira. (2025a). *Censo Escolar da Educação Básica 2024: Resumo técnico*. https://download.inep.gov.br/publicacoes/institucionais/estatisticas_e_indicadores/resumo_tecnico_censo_escolar_2024.pdf

Instituto Nacional de Estudos e Pesquisas Educacionais Anísio Teixeira. (2025b). *Censo da Educação Superior 2024: Notas estatísticas*. https://download.inep.gov.br/publicacoes/institucionais/estatisticas_e_indicadores/notas_estatisticas_censo_escolar_da_educacao_superior_2024.pdf

Ministry of Planning and Budget, Brazil. (2026). *Agenda Transversal Povos Indígenas 2026: Ano-base 2025*. https://www.gov.br/planejamento/pt-br/assuntos/orcamento/agendas-transversais-e-multisetoriais/arquivos/2026/2026_05_05_atpovosindigenas.pdf/@@display-file/file

OECD. (2023d). *PISA 2022 results: Brazil country note*. https://www.oecd.org/content/dam/oecd/en/publications/reports/2023/11/pisa-2022-results-volume-i-and-ii-country-notes_2fca04b9/brazil_764ee0aa/61690648-en.pdf

Asociación Nacional de Universidades e Instituciones de Educación Superior. (2026). *Anuario estadístico de educación superior*. https://www.anuies.mx/informacion-y-servicios/informacion-estadistica-de-educacion-superior/anuario-estadistico-de-educacion-superior

Cámara de Diputados. (2024). *Ley General para la Inclusión de las Personas con Discapacidad* (última reforma DOF 14-06-2024). https://www.diputados.gob.mx/LeyesBiblio/pdf/LGIPD.pdf

Cámara de Diputados. (2026). *Ley General de Educación* (última reforma DOF 15-01-2026). https://www.diputados.gob.mx/LeyesBiblio/pdf/LGE.pdf

Comisión Nacional de Libros de Texto Gratuitos. (2026). *Catálogo de libros, ciclo escolar 2026–2027*. https://www.conaliteg.sep.gob.mx/

Dirección General de Educación Indígena, Intercultural y Bilingüe. (2024). *Estadística indígena, ciclo 2023–2024*. Secretaría de Educación Pública.

Instituto Nacional de Estadística y Geografía. (2022). *Principales resultados del Censo de Población y Vivienda 2020: Estados Unidos Mexicanos*. https://www.inegi.org.mx/programas/ccpv/2020/

Gallaudet University Press. (n.d.). *The hidden treasure of Black ASL*. https://gupress.gallaudet.edu/Books/T/The-Hidden-Treasure-of-Black-ASL

National Advisory Council on Indian Education. (2023). *Annual report to Congress, 2022–2023*. https://www.ed.gov/sites/ed/files/2023/08/NACIE-AnnualReport2023-508_completed.pdf

National Center for Education Statistics. (2023a). *English learners in public schools*. Condition of Education. https://nces.ed.gov/programs/coe/pdf/2023/cgf_508.pdf

National Center for Education Statistics. (2023b). *Enrollment in public elementary and secondary schools, by level and grade: Selected years, fall 1980 through fall 2031* [Data set]. https://nces.ed.gov/programs/digest/d23/tables/xls/tabn203.10.xlsx

National Center for Education Statistics. (2024a). *Percentage of three-, four-, and five-year-old children enrolled in school, by age and selected child and family characteristics: 2022 and 2023* [Data set]. https://nces.ed.gov/programs/digest/d24/tables/xls/tabn202.20.xlsx

National Center for Education Statistics. (2024b). *Career and technical education in the United States*. https://nces.ed.gov/programs/coe/pdf/2024/tob_508c.pdf

National Center for Education Statistics. (2026). *Total fall enrollment in degree-granting postsecondary institutions, by level of enrollment, control and level of institution, attendance status, and sex of student: Selected years, 1970 through 2034* [Data set]. https://nces.ed.gov/programs/digest/d25/tables/xls/tabn306.10.xlsx

National Center for Science and Engineering Statistics. (2025). *Doctorate recipients from U.S. universities: 2023* (NSF 25-300). https://ncses.nsf.gov/pubs/nsf25300/assets/nsf25300.pdf

National Deaf Center on Postsecondary Outcomes. (2019). *Undergraduate enrollment of deaf students in the United States*. https://nationaldeafcenter.org/wp-content/uploads/2022/11/Undergraduate20Enrollment2020of20Deaf20Students20in20the20United20States.pdf

National Deaf Center on Postsecondary Outcomes. (2024). *Access, belonging, and affirmation: Deaf college student data*. https://nationaldeafcenter.org/resources/research-data/deaf-college-student-data/access-belonging-and-affirmation/

U.S. Census Bureau. (2022a). *Language use in the United States: 2019* (ACS-50). https://www.census.gov/content/dam/Census/library/publications/2022/acs/acs-50.pdf

U.S. Census Bureau. (2022b). *2017–2021 ACS language tables: Nation* [Data set]. https://www2.census.gov/programs-surveys/demo/tables/language-use/2017/2017-2021-acs-lang-tables-nation.xlsx

U.S. Census Bureau. (2022c). *2017–2021 ACS language tables: States* [Data set]. https://www2.census.gov/programs-surveys/demo/tables/language-use/2017/2017-2021-acs-lang-tables-state.xlsx

U.S. Census Bureau. (2023a). *2020 Island Areas Censuses detailed cross-tabulations: Technical documentation*. https://www2.census.gov/programs-surveys/decennial/2020/technical-documentation/island-areas-tech-docs/detailed-cross-tabulations/2020-iac-dct-technical-documentation.pdf

U.S. Census Bureau. (2023b). *2020 Island Areas Censuses detailed cross-tabulations: Guidance*. https://www2.census.gov/programs-surveys/decennial/2020/technical-documentation/island-areas-tech-docs/detailed-cross-tabulations/2020-iac-dct-guidance.pdf

U.S. Department of Education. (2025). *47th annual report to Congress on the implementation of the Individuals with Disabilities Education Act*. https://sites.ed.gov/idea/files/47th-ARC-for-IDEA.pdf

World Bank. (2026). *World Development Indicators: United States population, individuals using the Internet, and access to electricity* [Data set]. https://api.worldbank.org/v2/country/USA?format=json
